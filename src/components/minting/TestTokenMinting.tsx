...
const TestTokenMinting: React.FC = () => {
  ...
  const { loadTokenBalances, loadEoaTokenBalances } = useTokenStore();
  ...
  const handleMintTestToken = async () => {
    ...
    try {
      ...
      const paymentTypeNum = paymentType === 'SPONSORED' ? 0 : paymentType === 'PREPAY' ? 1 : 2;
      ...
      await loadTestTokenBalance();
      await loadTokenBalances(walletState.aaWalletAddress, useTokenStore.getState().supportedTokens);
      await loadEoaTokenBalances(
        walletState.userAddress,
        useTokenStore.getState().supportedTokens
      );
    } catch (error) {
  ...
  const handleTransferTestToken = async () => {
    ...
    try {
      const amountToTransfer = ethers.parseUnits(transferAmount, 18);
      const paymentTypeNum =
        transferPaymentType === 'SPONSORED' ? 0 : transferPaymentType === 'PREPAY' ? 1 : 2;
