...
import { checkTournamentParticipation } from '../lib/ethUtils';
...
const HoneyClicker = () => {
  ...
  useEffect(() => {
    async function check() {
      setLoading(true);
      try {
        const result = await checkTournamentParticipation(
          getProvider(),
    ...
    if (getProvider() && TESTNET_CONFIG.smartContracts.tournamentHub && aaWalletAddress) {
      check();
    }
  }, [aaWalletAddress]);
 ...
 useEffect(() => {
    if (loading) {
      ...
      setIsLoadingModalOpen(false); // Close modal when loading is done
    }
  }, [loading]);
              ...
              <Card className="bg-black border-cyan-400 border-2 p-6 mt-6">
                ...
                <div>
                  {isParticipant ? (
                    ...
                    <p>You must join the tournament to submit a score.</p>
                  )}
                </div>
