...
const Shop: React.FC<ShopProps> = ({ shopOpen, setShopOpen, onOpen, onClose, onBuy }) => {
  ...
  const [mintingSkin, setMintingSkin] = useState(null);
  ...
  const handleBuySkin = (item) => {
    setMintingSkin(item);
    setIsMintingModalOpen(true);
    // For demo: optionally call buyItem(item.id) here or after successful mint
    // buyItem(item.id);
  };
                  ...
                  <div className="flex items-center">
                    ...
                    {item.type === 'skin' ? (
                      <button
                        ...
                        disabled={Boolean(ownedItems[item.id])}
                      ...
                      >
                        {ownedItems[item.id] ? '✓ OWNED' : 'BUY & MINT'}
                      </button>
                    ) : (
                      <button
                        onClick={() => {
