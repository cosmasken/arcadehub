...
const TokenApproval: React.FC<TokenApprovalProps> = ({
  ...
  const [approvalStep, setApprovalStep] = useState<
    'check' | 'eoa-approval' | 'transfer' | 'aa-approval' | 'complete'
  >('check');
  ...
  const checkEOAAllowance = async () => {
    if (!walletState.EOAProvider || !walletState.eoaAddress || !selectedToken) return;
  ...
  const checkAllowances = async () => {
    if (!walletState.EOAProvider || !walletState.eoaAddress || !selectedToken || !walletState.signer) return;

    setIsCheckingAllowance(true);
    setApprovalError('');

    try {
      await Promise.all([checkEOAAllowance(), checkAAAllowance()]);
  ...
  const handleEOAApprove = async () => {
    ...
    try {
      ...
      await approveToken(walletState.EOAProvider as any, selectedToken, approvalAmount);
  ...
  const handleTransferToAAWallet = async () => {
    if (!walletState.EOAProvider || !walletState.eoaAddress || !selectedToken || !walletState.aaWalletAddress) return;
  ...
  const handleAAWalletApprove = async () => {
    ...
    try {
      const token = getTokenDetails();
      const decimals = token?.decimals || 18;
      const approvalAmount = ethers.parseUnits('10000000', decimals);
  ...
  }, [selectedToken, walletState.eoaAddress, walletState.aaWalletAddress]);
    ...
    <div className="token-approval">
      <div className="allowance-status">
        {isCheckingAllowance ? (
          ...
          <div className="allowance-required">
            <p>
              You need to approve the token to pay for gas with{' '}
              {getTokenDetails()?.symbol || 'selected token'}
            </p>
            <button
              onClick={handleAAWalletApprove}
              disabled={!!isApprovingAAToken}
