...
const AdminDashboard: React.FC<{ onLogout: () => void }> = ({ onLogout }) => {
  ...
  const fetchInitialData = useCallback(async () => {
    try {
      ...
      // For each player, fetch all claims using getClaimsCount and getClaim
      ...
      setPendingClaims(claims.sort((a, b) => b.blockNumber - a.blockNumber));
  ...
  useEffect(() => {
    ...
    const handlePointsClaimSubmitted = (player: string, claimIndex: ethers.BigNumberish, points: ethers.BigNumberish) => {
      setPendingClaims(prev => {
        ...
        return [...prev, { player, claimIndex: Number(claimIndex), points: Number(points), blockNumber: 0 }]
