...
// ====== USER: SET THESE ======
const PRIVATE_KEY = process.env.VITE_PRIVATE_KEY || ""; // Or paste your private key here for testing
const NATIVE_TOKEN = ethers.ZeroAddress; // Use 0x0 for native
const ARCADE_TOKEN = TESTNET_CONFIG.smartContracts.arcadeToken; // Your custom token address
...
const tournamentHub = new ethers.Contract(TOURNAMENT_HUB, TournamentHubABI, wallet);
    ...
    try {
      ...
      if (t.useNative) {
        ...
        tx = await tournamentHub.createTournament(
          ...
          t.tournamentType,
          t.challengeParam,
          { value: 0 } // No native value, token transfer handled by ERC20 approve/transferFrom if supported
        );
      }
